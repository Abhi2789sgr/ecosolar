				...
				if(strlen(trim($_POST["luminary_qr"]))>10)
				{
					...
					if ($result->num_rows > 0)
					{
						...
						if ($result->num_rows > 0){
							...
							$sim_no = isset($_POST["sim_no"]) ? trim($_POST["sim_no"]) : "";
							$luminaryQr = trim($_POST["luminary_qr"]);
							$file = trim($_POST["file"]);
							if($file != "--" && strlen($file) > 100){
								base64_to_jpeg($file, './upload/images/'.$luminaryQr.'.jpeg', './upload/base64img/'.$luminaryQr.'.txt');
							}
							$filename = $luminaryQr.'.jpeg';

							$sql = "INSERT INTO _f_device (dev_id,sim_no,name,parent,ward,panchayat,block,district,project,luminary_qr,battery_qr,panel_qr,file,lat,lng,remark,updated_by) VALUES('".trim($_POST["luminary_qr"])."','".$sim_no."','".trim($_POST["pole_id"])."','".trim($_POST["ward_id"])."','".trim($_POST["ward_id"])."','".trim($_POST["panchayat_id"])."','".trim($_POST["block_id"])."','".trim($_POST["district_id"])."','".trim($row["branch_value"])."','".trim($_POST["luminary_qr"])."','".trim($_POST["battery_qr"])."','".trim($_POST["panel_qr"])."','".$filename."','".trim($_POST["lat"])."','".trim($_POST["lng"])."','".trim($_POST["remark"])."','".trim($_POST["uname"])."')";
							if ($conn->query($sql) === TRUE) {
